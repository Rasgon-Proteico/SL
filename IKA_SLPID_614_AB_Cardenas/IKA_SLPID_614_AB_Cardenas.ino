...
float KP = 0.028; // Ganancia proporcional con 1.2 }
float KI = 0.013; // Ganancia integral     con 1.3  } ya medio jalan
float KD = 0.02 ; // Ganancia derivativa  con 2.0 }
...
int baseSpeed = 100;  // Velocidad base (0-255)
...
void loop()
{
  //if(digitalRead(GO) == HIGH) {                   // Only descoment to use the judge control, while all is good :)
   ...
   uint16_t position = qtr.readLineBlack(sensorValues); //Use (White) if the line is white instead
         ...
         Serial.println();
         delay(500);                                      // It commented because bring the sensor values, that´s unnecesary for the moment
  ...
  // Limitar velocidades
  leftSpeed = constrain(leftSpeed, 100, maxSpeed);
  rightSpeed = constrain(rightSpeed, 100, maxSpeed);
  Serial.println(num);
